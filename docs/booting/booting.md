...
* `debug` sets the log level to debug, it also sets `sync` flag on the log file so if the system crashed for an unknown reason we make sure that the crash logs is committed to the permanent storage. When debug is not set firewall rules will be applied so that the redis port and ssh are only available via the zerotier network.
