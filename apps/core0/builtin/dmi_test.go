...
func TestParseTestOEMSpecificTypeWithIndentedListItem(t *testing.T) {
	...
	if ok := assert.Len(t, dmi["Custom Type 216"].SubSections, 1); !ok {
		t.Fatal()
	}
	if ok := assert.Equal(t, dmi["Custom Type 216"].Type, DMIType(216)); !ok {
		t.Fatal()
	}
	if ok := assert.Len(t, oemSpecificTypeWithIndentedListItems, 2); !ok {
		t.Fatal()
	}
	if ok := assert.Len(t, dmi["Custom Type 216"].SubSections["OEM-specific Type"], 2); !ok {
		t.Fatal()
	}
	for k, property := range oemSpecificTypeWithIndentedListItems {
		foundProperty, ok := dmi["Custom Type 216"].SubSections["OEM-specific Type"][k]
		if !ok {
			t.Fatal()
		}
		if ok := assert.Equal(t, property.Val, foundProperty.Val); !ok {
