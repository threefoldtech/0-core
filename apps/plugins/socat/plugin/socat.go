...
//with one call later.
func (s *socatManager) SetPortForward(ns socat.NS, ip string, host string, dest int) error {
	s.rm.Lock()
	defer s.rm.Unlock()
...
func (s *socatManager) RemovePortForward(ns socat.NS, host string, dest int) error {
	s.rm.Lock()
	defer s.rm.Unlock()
	src, err := getSource(host)
	if err != nil {
...
func (s *socatManager) RemoveAll(ns socat.NS) error {
	s.rm.Lock()
	defer s.rm.Unlock()
